...
const PixelBlast: React.FC<PixelBlastProps & { onLoad?: () => void }> = ({
  variant = "square",
  pixelSize = 3,
  color = "#B19EEF",
  className,
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Call onLoad callback when component is ready
      if (onLoad) {
        setTimeout(onLoad, 100) // Small delay to ensure everything is rendered
      }
    } else {
      const t = threeRef.current!
